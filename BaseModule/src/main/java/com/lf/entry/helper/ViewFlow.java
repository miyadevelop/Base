...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private void snapToScreen(int whichScreen) {
		...
		if (!mScroller.isFinished())
			return;
		//		MyLog.i("view flow snapToScreen: whichScreen = " + whichScreen + " getChildCount() - 1 = " + (getChildCount() - 1));
		whichScreen = Math.max(0, Math.min(whichScreen, getChildCount() - 1));

		mNextScreen = whichScreen;
		//		MyLog.i("view flow snapToScreen: next = " + mNextScreen + " cur = " + mCurrentScreen);

		final int newX = whichScreen * getWidth();
		final int delta = newX - getScrollX();
		mScroller.startScroll(getScrollX(), 0, delta, 0, mAutoFlowTime/4);
		invalidate();
	}
	...
	public void computeScroll() {
		if (mScroller.computeScrollOffset()) {
			...
			//			MyLog.i("view flow computeScroll: next = " + mNextScreen + " cur = " + mCurrentScreen);
	...
	private void postViewSwitched(int direction) {
		...
		if (direction > 0) { // to the right
			mCurrentAdapterIndex = direction + mCurrentAdapterIndex;
			mCurrentBufferIndex = direction + mCurrentBufferIndex;
	...
	public void startAutoFlow(){
		mHandler = new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				case 1:
					...
					//					MyLog.i("view flow handleMessage: next = " + mNextScreen + " cur = " + mCurrentScreen);
					if(getChildCount() != 0)
						snapToScreen((mCurrentScreen+1)%getChildCount());
					Message message = mHandler.obtainMessage(1);
					mHandler.removeMessages(1);
					sendMessageDelayed(message, mAutoFlowTime);		
					break;
		...
		Message message = mHandler.obtainMessage(1);
		mHandler.sendMessageDelayed(message, mAutoFlowTime);
	}
	 ...
	 * 设置自动滚动间隔
	 */
	private int mAutoFlowTime = 5000;
	public void setAutoFlowTime(int time)
	{
		mAutoFlowTime = time;
	}
	...
	/**
	 * 设置缓存的项数，需要在获取到数据后进行调用，否则自动滚动的动画将会异常
	 * @param buffer
	 */
	public void setSideBuffer(int buffer)
	{
		mSideBuffer = buffer;
	}
